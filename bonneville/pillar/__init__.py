...
from bonneville.template import compile_template
from bonneville.utils.dictupdate import update
from bonneville.utils.odict import OrderedDict
...
class RemotePillar(object):
    ...
    def compile_pillar(self):
        ...
        pcrypt = bonneville.crypt.Crypticle(self.opts, aes)
...
class Pillar(object):
    ...
    def __init__(self, opts, grains, id_, env, ext=None):
        ...
        self.client = bonneville.fileclient.get_file_client(self.opts)
        if opts.get('file_client', '') == 'local':
            opts['grains'] = grains
            self.functions = bonneville.loader.minion_mods(opts)
        else:
            self.functions = bonneville.loader.minion_mods(self.opts)
        self.matcher = bonneville.minion.Matcher(self.opts, self.functions)
        ...
        self.ext_pillars = bonneville.loader.pillars(ext_pillar_opts, self.functions)
