# -*- coding: utf-8 -*-
'''
The module used to execute states in bonneville. A state is unlike a module
...
import bonneville.fileclient
import bonneville.utils.event
import bonneville.syspaths as syspaths
from bonneville._compat import string_types
from bonneville.template import compile_template, compile_template_str
from bonneville.exceptions import SaltRenderError, SaltReqTimeoutError, SaltException
...
class Compiler(object):
    ...
    def __init__(self, opts):
        self.opts = opts
        self.rend = bonneville.loader.render(self.opts, {})
...
class State(object):
    ...
    def _gather_pillar(self):
        ...
        '''
        pillar = bonneville.pillar.get_pillar(
                self.opts,
                self.opts['grains'],
                self.opts['id'],
    ...
    def load_modules(self, data=None):
        ...
        self.functions = bonneville.loader.minion_mods(self.opts, self.state_con)
        if isinstance(data, dict):
            if data.get('provider', False):
                ...
                for provider in providers:
                    for mod in provider:
                        funcs = bonneville.loader.raw_mod(self.opts,
                                provider[mod],
                                self.functions)
                        if funcs:
                            for func in funcs:
    ...
    def verify_data(self, data):
        ...
        if full not in self.states:
            ...
            aspec = bonneville.utils.get_function_argspec(self.states[full])
            arglen = 0
            deflen = 0
    ...
    def format_call(self, data):
        ...
        ret['args'] = []
        aspec = bonneville.utils.get_function_argspec(self.states[ret['full']])
        arglen = 0
        deflen = 0
        if isinstance(aspec.args, list):
            arglen = len(aspec.args)
    ...
    def event(self, chunk_ret):
        ...
        if not self.opts.get('local') and self.opts.get('state_events', True) and self.opts.get('master_uri'):
            tag = bonneville.utils.event.tagify(
    ...
    def call_highstate(self, exclude=None, cache=None, cache_name='highstate',
        ...
        if cache:
            if os.path.isfile(cfn):
                with bonneville.utils.fopen(cfn, 'rb') as fp_:
...
class HighState(BaseHighState):
    ...
    def __init__(self, opts, pillar=None):
        self.client = bonneville.fileclient.get_file_client(opts)
        BaseHighState.__init__(self, opts)
        self.state = State(self.opts, pillar)
        self.matcher = bonneville.minion.Matcher(self.opts)
...
class MasterState(State):
    ...
    def load_modules(self, data=None):
        ...
        #
        self.functions = bonneville.client.FunctionWrapper(
                self.opts,
                self.opts['id']
                )
...
class RemoteHighState(object):
    ...
    def __init__(self, opts, grains):
        ...
        self.grains = grains
        self.serial = bonneville.payload.Serial(self.opts)
        self.auth = bonneville.crypt.SAuth(opts)
        self.sreq = bonneville.payload.SREQ(self.opts['master_uri'])
