...
class Client(object):
    ...
    def cache_dir(self, path, env='base', include_empty=False):
        ...
        if include_empty:
            ...
            #else:
            #    prefix = separated[0]
            dest = bonneville.utils.path_join(
                self.opts['cachedir'],
                'files',
                env
            )
    ...
    def get_url(self, url, dest, makedirs=False, env='base'):
        ...
        try:
            with contextlib.closing(url_open(fixed_url)) as srcfp:
                with bonneville.utils.fopen(dest, 'wb') as destfp:
    ...
    def get_template(
        ...
        if not os.path.isdir(destdir):
            if makedirs:
                ...
                bonneville.utils.safe_rm(data['data'])
                return ''
...
class RemoteClient(Client):
    ...
    def _crypted_transfer(self, load, tries=3, timeout=60, payload='aes'):
        ...
        try:
            return _do_transfer()
        except bonneville.crypt.AuthenticationError:
            self.auth = bonneville.crypt.SAuth(self.opts)
            return _do_transfer()
        ...
        if dest:
            ...
            if not os.path.isdir(destdir):
                if makedirs:
                    ...
                    return False
            fn_ = bonneville.utils.fopen(dest, 'wb+')
        while True:
            if not fn_:
                load['loc'] = 0
            else:
            ...
            if not fn_:
                with self._cache_loc(data['dest'], env) as cache_dest:
                    ...
                    if os.path.isdir(dest):
                        bonneville.utils.rm_rf(dest)
                    fn_ = bonneville.utils.fopen(dest, 'wb+')
            if data.get('gzip', None):
                data = bonneville.utils.gzip_util.uncompress(data['data'])
            else:
                data = data['data']
            fn_.write(data)
        if fn_:
    ...
    def hash_file(self, path, env='base'):
        ...
        try:
            ...
            if not os.path.isfile(path):
                ...
                ret = {}
                ret['hsum'] = bonneville.utils.get_hash(
