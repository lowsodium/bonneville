...
import bonneville.utils.network
import bonneville.pillar
import bonneville.syspaths
...
DEFAULT_MINION_OPTS = {
    ...
    'master_finger': '',
    'user': 'root',
    'root_dir': bonneville.syspaths.ROOT_DIR,
    'pki_dir': os.path.join(bonneville.syspaths.CONFIG_DIR, 'pki', 'minion'),
    'id': None,
    'cachedir': os.path.join(bonneville.syspaths.CACHE_DIR, 'minion'),
    'cache_jobs': False,
    'conf_file': os.path.join(bonneville.syspaths.CONFIG_DIR, 'minion'),
    'sock_dir': os.path.join(bonneville.syspaths.SOCK_DIR, 'minion'),
    'backup_mode': '',
    'renderer': 'yaml_jinja',
    'failhard': False,
    ...
    'file_client': 'remote',
    'file_roots': {
        'base': [bonneville.syspaths.BASE_FILE_ROOTS_DIR],
    },
    'fileserver_limit_traversal': False,
    'pillar_roots': {
        'base': [bonneville.syspaths.BASE_PILLAR_ROOTS_DIR],
    },
    'hash_type': 'md5',
    'external_nodes': '',
    'disable_modules': [],
    ...
    'tcp_pub_port': 4510,
    'tcp_pull_port': 4511,
    'log_file': os.path.join(bonneville.syspaths.LOGS_DIR, 'minion'),
    'log_level': None,
    ...
    'pidfile': os.path.join(bonneville.syspaths.PIDFILE_DIR, 'salt-minion.pid'),
...
DEFAULT_MASTER_OPTS = {
    ...
    'user': 'root',
    'worker_threads': 5,
    'sock_dir': os.path.join(bonneville.syspaths.SOCK_DIR, 'master'),
    'ret_port': '4506',
    'timeout': 5,
    'keep_jobs': 24,
    'root_dir': bonneville.syspaths.ROOT_DIR,
    'pki_dir': os.path.join(bonneville.syspaths.CONFIG_DIR, 'pki', 'master'),
    'cachedir': os.path.join(bonneville.syspaths.CACHE_DIR, 'master'),
    'file_roots': {
        'base': [bonneville.syspaths.BASE_FILE_ROOTS_DIR],
    },
    'master_roots': {
        'base': [bonneville.syspaths.BASE_MASTER_ROOTS_DIR],
    },
    'pillar_roots': {
        'base': [bonneville.syspaths.BASE_PILLAR_ROOTS_DIR],
    },
    ...
    'hash_type': 'md5',
    'conf_file': os.path.join(bonneville.syspaths.CONFIG_DIR, 'master'),
    'open_mode': False,
    'auto_accept': False,
    'renderer': 'yaml_jinja',
    'failhard': False,
    ...
    'ipv6': False,
    'log_file': os.path.join(bonneville.syspaths.LOGS_DIR, 'master'),
    'log_level': None,
    'log_level_logfile': None,
    'log_datefmt': _DFLT_LOG_DATEFMT,
    'log_datefmt_logfile': _DFLT_LOG_DATEFMT_LOGFILE,
    ...
    'log_fmt_logfile': _DFLT_LOG_FMT_LOGFILE,
    'log_granular_levels': {},
    'pidfile': os.path.join(bonneville.syspaths.PIDFILE_DIR, 'salt-master.pid'),
    ...
    'key_logfile': os.path.join(bonneville.syspaths.LOGS_DIR, 'key'),
    'verify_env': True,
    'permissive_pki_access': False,
    'default_include': 'master.d/*.conf',
    'win_repo': os.path.join(bonneville.syspaths.BASE_FILE_ROOTS_DIR, 'win', 'repo'),
    'win_repo_mastercachefile': os.path.join(bonneville.syspaths.BASE_FILE_ROOTS_DIR,
                                             'win', 'repo', 'winrepo.p'),
...
def _validate_file_roots(opts):
    ...
    if not isinstance(opts['file_roots'], dict):
        log.warning('The file_roots parameter is not properly formatted,'
                    ' using defaults')
        return {'base': [bonneville.syspaths.BASE_FILE_ROOTS_DIR]}
...
def _read_conf_file(path):
    ...
    with bonneville.utils.fopen(path, 'r') as conf_file:
        try:
...
def load_config(path, env_var, default_path=None):
    ...
    if default_path is None:
        ...
        log.warning(
            ...
            'new \'default_path\' argument to `bonneville.config.load_config()`. '
    ...
    if not os.path.isfile(path):
        ...
        if os.path.isfile(template):
            import bonneville.utils  # TODO: Need to re-import, need to find out why
            log.debug('Writing {0} based on {1}'.format(path, template))
            with bonneville.utils.fopen(path, 'w') as out:
                with bonneville.utils.fopen(template, 'r') as ifile:
...
def prepend_root_dir(opts, path_options):
    ...
    for path_option in path_options:
        if path_option in opts:
            ...
            opts[path_option] = bonneville.utils.path_join(
                root_dir,
                opts[path_option]
            )
...
def syndic_config(master_config_path,
    ...
    syndic_opts = {
        'root_dir': opts.get('root_dir', bonneville.syspaths.ROOT_DIR),
...
def get_id(root_dir=None, minion_id=False, cache=True):
    ...
    config_dir = bonneville.syspaths.CONFIG_DIR
    if config_dir.startswith(bonneville.syspaths.ROOT_DIR):
        config_dir = config_dir.split(bonneville.syspaths.ROOT_DIR, 1)[-1]
    ...
    if cache:
        try:
            with bonneville.utils.fopen(id_cache) as idf:
                name = idf.read().strip()
            if name:
    ...
    log.debug('Guessing ID. The id can be explicitly in set {0}'
              .format(os.path.join(bonneville.syspaths.CONFIG_DIR, 'minion')))
    ...
    if fqdn != 'localhost':
        ...
        if minion_id and cache:
            try:
                with bonneville.utils.fopen(id_cache, 'w') as idf:
                    idf.write(fqdn)
    ...
    # Check /etc/hostname
    try:
        with bonneville.utils.fopen('/etc/hostname') as hfl:
            name = hfl.read().strip()
    ...
    try:
        with bonneville.utils.fopen('/etc/hosts') as hfl:
            for line in hfl:
                names = line.split()
                ip_ = names.pop(0)
    ...
    try:
        ...
        with bonneville.utils.fopen(windir + r'\system32\drivers\etc\hosts') as hfl:
            for line in hfl:
    ...
    ip_addresses = [bonneville.utils.network.IPv4Address(addr) for addr
                    in bonneville.utils.network.ip_addrs(include_loopback=True)
...
def apply_master_config(overrides=None, defaults=None):
    ...
    opts['aes'] = bonneville.crypt.Crypticle.generate_key_string()
    ...
    # to make `bonneville.modules.publish` not work under the test-suite.
...
def client_config(path, env_var='SALT_CLIENT_CONFIG', defaults=None):
    ...
    if os.path.isfile(opts['token_file']):
        with bonneville.utils.fopen(opts['token_file']) as fp_:
