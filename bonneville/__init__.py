...
warnings.filterwarnings(
    ...
    r'^(salt|salt\.(.*))$'  # Match module(s) 'salt' and 'bonneville.<whatever>'
)
...
# instantiates is using bonneville.log.setup.SaltLoggingClass
import bonneville.log.setup


# the try block below bypasses an issue at build time so that modules don't
# cause the build to fail
from bonneville.version import __version__
from bonneville.utils import migrations

try:
    from bonneville.utils import parsers, ip_bracket
    from bonneville.utils.verify import check_user, verify_env, verify_socket
    from bonneville.utils.verify import verify_files
except ImportError as exc:
    if exc.args[0] != 'No module named _msgpack':
        raise
from bonneville.exceptions import SaltSystemExit, MasterExit


# Let's instantiate logger using bonneville.log.setup.logging.getLogger() so pylint
# leaves us alone and stops complaining about an un-used import
logger = bonneville.log.setup.logging.getLogger(__name__)
...
class Master(parsers.MasterOptionParser):
    ...
    def prepare(self):
        ...
        self.master = bonneville.master.Master(self.config)
...
class Minion(parsers.MinionOptionParser):
    ...
    def prepare(self):
        ...
        if isinstance(self.config.get('master'), list):
            self.minion = bonneville.minion.MultiMinion(self.config)
        else:
            self.minion = bonneville.minion.Minion(self.config)
...
class Syndic(parsers.SyndicOptionParser):
    ...
    def prepare(self):
        ...
        import bonneville.minion
        self.daemonize_if_required()
        self.syndic = bonneville.minion.Syndic(self.config)
        self.set_pidfile()
