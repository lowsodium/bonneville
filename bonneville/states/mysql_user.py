...
:configuration: See :py:mod:`bonneville.modules.mysql` for setup instructions.
...
def present(name,
    ...
    if passwordless:
        if not bonneville.utils.is_true(allow_passwordless):
    ...
    if __salt__['mysql.user_exists'](name, host, **connection_args):
        ...
        if __salt__['mysql.user_chpass'](name, host,
            ...
            if passwordless and not bonneville.utils.is_true(allow_passwordless):
