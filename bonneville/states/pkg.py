...
from bonneville.exceptions import CommandExecutionError
from bonneville.modules.pkg_resource import _repack_pkgs

if bonneville.utils.is_windows():
    from bonneville.utils import namespaced_function as _namespaced_function
    from bonneville.modules.win_pkg import _get_package_info
    from bonneville.modules.win_pkg import get_repo_data
    from bonneville.modules.win_pkg import _get_latest_pkg_version
    from bonneville.modules.win_pkg import _reverse_cmp_pkg_versions
...
def _fulfills_version_spec(versions, oper, desired_version):
    ...
    '''
    for ver in versions:
        if bonneville.utils.compare_versions(ver1=ver,
                                       oper=oper,
...
def installed(
    ...
    version
        ...
        for the following pkg providers: :mod:`apt <bonneville.modules.apt>`,
        :mod:`ebuild <bonneville.modules.ebuild>`,
        :mod:`pacman <bonneville.modules.pacman>`,
        :mod:`yumpkg <bonneville.modules.yumpkg>`,
        :mod:`yumpkg5 <bonneville.modules.yumpkg5>`, and
        :mod:`zypper <bonneville.modules.zypper>`.
    ...
    ``NOTE:`` For :mod:`apt <bonneville.modules.apt>`,
    :mod:`ebuild <bonneville.modules.ebuild>`,
    :mod:`pacman <bonneville.modules.pacman>`, :mod:`yumpkg <bonneville.modules.yumpkg>`,
    :mod:`yumpkg5 <bonneville.modules.yumpkg5>`,
    and :mod:`zypper <bonneville.modules.zypper>`, version numbers can be specified
    ...
    Additionally, :mod:`ebuild <bonneville.modules.ebuild>`,
    :mod:`pacman <bonneville.modules.pacman>` and
    :mod:`zypper <bonneville.modules.zypper>` support the ``<``, ``<=``, ``>=``, and
    ...
    With :mod:`ebuild <bonneville.modules.ebuild>` is also possible to specify a use
    ...
    if bonneville.utils.is_true(refresh) or os.path.isfile(rtag):
...
def latest(
    ...
    :mod:`installed <bonneville.states.pkg.installed>` function to be
    used, as :mod:`latest <bonneville.states.pkg.latest>` will update the package
    ...
    for pkg in desired_pkgs:
        if not avail[pkg]:
            if not cur[pkg]:
                ...
                or bonneville.utils.compare_versions(
                    ver1=cur[pkg],
                    oper='<',
                    ver2=avail[pkg],
                    cmp_func=__salt__.get('version_cmp')):
...
def mod_init(low):
    ...
    .. seealso::
       :py:func:`bonneville.modules.ebuild.ex_mod_init`
    ...
    if low['fun'] == 'installed' or low['fun'] == 'latest':
        ...
        if not os.path.exists(rtag):
            bonneville.utils.fopen(rtag, 'w+').write('')
        return ret
    return False
