...
def _get_template_texts(source_list=None,
    ...
    for (source, source_hash) in source_list:
        ...
        if rndrd_templ_fn:
            tmplines = None
            with bonneville.utils.fopen(rndrd_templ_fn, 'rb') as fp_:
...
def managed(name,
    ...
    contents_pillar
        ...
        <bonneville.modules.pillar.get>`. This is useful when the pillar value
...
def recurse(name,
    ...
    ) for (k, v) in ret['comment'].items()).strip()
...
def replace(name,
    ...
    Params are identical to :py:func:`~bonneville.modules.replace`.
...
def sed(name,
    ...
    with bonneville.utils.fopen(name, 'rb') as fp_:
    ...
    if slines != nlines:
        if not bonneville.utils.istextfile(name):
...
def append(name,
    ...
    for chunk in text:
        ...
        if __salt__['file.contains_regex_multiline'](
                name, bonneville.utils.build_whitespace_split_regex(chunk)):
            continue
