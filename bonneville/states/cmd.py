...
Should I use :mod:`cmd.run <bonneville.states.cmd.run>` or :mod:`cmd.wait <bonneville.states.cmd.wait>`?
--------------------------------------------------------------------------------------------

These two states are often confused. The important thing to remember about them
is that :mod:`cmd.run <bonneville.states.cmd.run>` states are run each time the SLS
...
<bonneville.states.cmd.wait>` does just that. :mod:`cmd.wait <bonneville.states.cmd.wait>`
...
def _reinterpreted_state(state):
    '''
    Re-interpret the state returned by bonneville.sate.run using our protocol.
    '''
    ret = state['changes']
    state['changes'] = {}
    state['comment'] = ''
...
def script(name,
    ...
    if isinstance(env, string_types):
        ...
        bonneville.utils.warn_until((0, 19), msg)
...
def call(name,
    ...
    :mod:`bonneville.renderers.pydsl` renderer.

    The interpretation of `onlyif` and `unless` arguments are identical to
    those of :func:`bonneville.states.cmd.run`, and all other arguments(`cwd`,
