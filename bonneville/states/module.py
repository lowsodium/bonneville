...
call a single module function use a :mod:`module.run <bonneville.states.module.run>`
state:
...
for this the :mod:`module.wait <bonneville.states.module.wait>` state can be used:
...
def wait(name, **kwargs):
    ...
    .. note::
        Like the :mod:`cmd.run <bonneville.states.cmd.run>` state, this state will
...
def run(name, **kwargs):
    ...
    aspec = bonneville.utils.get_function_argspec(__salt__[name])
    ...
    if 'returner' in kwargs:
        ...
        returners = bonneville.loader.returners(__opts__, __salt__)
