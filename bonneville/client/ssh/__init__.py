...
import bonneville.client.ssh.shell
import bonneville.client.ssh.wrapper
import bonneville.utils
import bonneville.utils.thin
import bonneville.roster
...
EOF'''.format(bonneville.__version__, RSTR)
...
class SSH(object):
    ...
    def __init__(self, opts):
        ...
        self.roster = bonneville.roster.Roster(opts)
        ...
        if not os.path.isfile(priv):
            bonneville.client.ssh.shell.gen_key(priv)
        self.defaults = {
    ...
    def verify_env(self):
        ...
        '''
        if not bonneville.utils.which('sshpass'):
    ...
    def _key_deploy_run(self, host, target, re_run=True):
        ...
        single = Single(
                ...
                host,
                **target)
        if bonneville.utils.which('ssh-copy-id'):
        ...
        if re_run:
            ...
            try:
                data = bonneville.utils.find_json(stdout)
    ...
    def run(self):
        ...
        for ret in self.handle_ssh():
            ...
            bonneville.output.display_output(
                    ret,
...
class Single(object):
    ...
    def deploy(self):
        ...
        Deploy salt-thin
        '''
        thin = bonneville.utils.thin.gen_thin(self.opts['cachedir'])
        self.shell.send(
                thin,
    ...
    def run_wfunc(self):
        ...
        if refresh:
            ...
            pre_wrapper = bonneville.client.ssh.wrapper.FunctionWrapper(
                self.opts,
                self.id,
                **self.target)
        ...
        wrapper = bonneville.client.ssh.wrapper.FunctionWrapper(
            opts,
            self.id,
            **self.target)
        self.wfuncs = bonneville.loader.ssh_wrapper(opts, wrapper)
    ...
    def cmd(self):
        ...
        thin_sum = bonneville.utils.thin.thin_sum(
                self.opts['cachedir'],
                self.opts['hash_type'])
        cmd = SSH_SHIM.format(
                sudo,
...
class SSHState(bonneville.state.State):
    '''
...
class SSHHighState(bonneville.state.BaseHighState):
    '''
    ...
    def __init__(self, opts, pillar=None, wrapper=None):
        self.client = bonneville.fileclient.LocalClient(opts)
        bonneville.state.BaseHighState.__init__(self, opts)
...
def prep_trans_tar(opts, chunks, file_refs):
    ...
    trans_tar = bonneville.utils.mkstemp()
    file_client = bonneville.fileclient.LocalClient(opts)
