...
from bonneville.exceptions import SaltInvocationError
from bonneville.exceptions import EauthAuthenticationError
...
class LocalClient(object):
    ...
    <all-bonneville.modules>` and return the results to the Salt Master.
    ...
    def __init__(self,
        ...
        if mopts:
            ...
            self.opts = bonneville.config.client_config(c_path)
        self.serial = bonneville.payload.Serial(self.opts)
        self.salt_user = self.__get_user()
        self.key = self.__read_master_key()
        self.event = bonneville.utils.event.LocalClientEvent(self.opts['sock_dir'])
    ...
    def __read_master_key(self):
        ...
        bonneville.utils.verify.check_path_traversal(self.opts['cachedir'], key_user)

        try:
            with bonneville.utils.fopen(keyfile, 'r') as key:
                return key.read()
    ...
    def cmd_batch(
        ...
        for key, val in self.opts.items():
            if key not in opts:
                opts[key] = val
        batch = bonneville.cli.batch.Batch(opts, True)
    ...
    def cmd(
        ...
        .. code:: python
            ...
            client = bonneville.client.LocalClient()
    ...
    def get_cli_returns(
        ...
        fret = {}
        inc_timeout = timeout
        jid_dir = bonneville.utils.jid_dir(jid,
                                     self.opts['cachedir'],
                                     self.opts['hash_type'])
        start = int(time.time())
        found = set()
                    ...
                    while fn_ not in ret:
                        try:
                            ...
                            ret_data = self.serial.load(
                                bonneville.utils.fopen(retp, 'rb')
                            )
                            if ret_data is None:
                            ...
                            if os.path.isfile(outp):
                                ret[fn_]['out'] = self.serial.load(
                                    bonneville.utils.fopen(outp, 'rb')
                                )
                        except Exception:
                            pass
                    found.add(fn_)
    ...
    def pub(self,
        ...
        if expr_form == 'nodegroup':
            ...
            tgt = bonneville.utils.minions.nodegroup_comp(tgt,
        ...
        sreq = bonneville.payload.SREQ(
            ...
            'tcp://' + bonneville.utils.ip_bracket(self.opts['interface']) +
...
class SSHClient(object):
    ...
    def _prep_ssh(
        ...
        opts['tgt'] = tgt
        opts['arg'] = arg
        return bonneville.client.ssh.SSH(opts)
...
class Caller(object):
    ...
    .. code-block:: python
        ...
        import bonneville.client
        caller = bonneville.client.Caller()
    ...
    def __init__(self, c_path=os.path.join(syspaths.CONFIG_DIR, 'minion')):
        self.opts = bonneville.config.minion_config(c_path)
        self.sminion = bonneville.minion.SMinion(self.opts)
    ...
    def function(self, fun, *args, **kwargs):
        ...
        args, kwargs = bonneville.minion.parse_args_and_kwargs(func, args, kwargs)
