...
import bonneville.utils.atomicfile
import bonneville.utils.event
import bonneville.utils.verify
import bonneville.utils.minions
import bonneville.utils.gzip_util
from bonneville.utils.debug import enable_sigusr1_handler, inspect_stack
from bonneville.exceptions import SaltMasterError, MasterExit
...
class SMaster(object):
    ...
    def __init__(self, opts):
        ...
        '''
        self.opts = opts
        self.master_key = bonneville.crypt.MasterKeys(self.opts)
    ...
    def __prep_crypticle(self):
        ...
        '''
        return bonneville.crypt.Crypticle(self.opts, self.opts['aes'])
    ...
    def __prep_key(self):
        ...
        for user in acl_users:
            ...
            key = bonneville.crypt.Crypticle.generate_key_string()
            with bonneville.utils.fopen(keyfile, 'w+') as fp_:
                fp_.write(key)
            os.umask(cumask)
...
class Master(SMaster):
    ...
    def _clear_old_jobs(self):
        ...
        search = bonneville.search.Search(self.opts)
        last = int(time.time())
        rotate = int(time.time())
        fileserver = bonneville.fileserver.Fileserver(self.opts)
        runners = bonneville.loader.runner(self.opts)
        schedule = bonneville.utils.schedule.Schedule(self.opts, runners)
        while True:
            now = int(time.time())
                    ...
                    for top in os.listdir(jid_root):
                        ...
                        for final in os.listdir(t_path):
                            ...
                            if not os.path.isfile(jid_file):
                                continue
                            with bonneville.utils.fopen(jid_file, 'r') as fn_:
                                jid = fn_.read()
                            if len(jid) < 18:
            ...
            if self.opts.get('publish_session'):
                if now - rotate >= self.opts['publish_session']:
                    bonneville.crypt.dropfile(self.opts['cachedir'])
                    rotate = now
            ...
            bonneville.utils.verify.check_max_open_files(self.opts)
...
class ReqServer(object):
    ...
    def start_event_publisher(self):
        ...
        '''
        # Start the publisher
        self.eventpublisher = bonneville.utils.event.EventPublisher(self.opts)
    ...
    def start_reactor(self):
        ...
        if self.opts.get('reactor'):
            self.reactor = bonneville.utils.event.Reactor(self.opts)
            self.reactor.start()
...
class MWorker(multiprocessing.Process):
    ...
    def _update_aes(self):
        ...
        if stats.st_mtime > self.k_mtime:
            ...
            with bonneville.utils.fopen(dfn) as fp_:
                aes = fp_.read()
            if len(aes) != 76:
                return
            self.crypticle = bonneville.crypt.Crypticle(self.opts, aes)
...
class AESFuncs(object):
    ...
    def __init__(self, opts, crypticle):
        self.opts = opts
        self.event = bonneville.utils.event.MasterEvent(self.opts['sock_dir'])
        self.serial = bonneville.payload.Serial(opts)
        self.crypticle = crypticle
        self.ckminions = bonneville.utils.minions.CkMinions(opts)
        # Create the tops dict for loading external top data
        self.tops = bonneville.loader.tops(self.opts)
        # Make a client
        self.local = bonneville.client.LocalClient(self.opts['conf_file'])
    ...
    def __setup_fileserver(self):
        ...
        '''
        fs_ = bonneville.fileserver.Fileserver(self.opts)
    ...
    def __verify_minion(self, id_, token):
        ...
        '''
        if not bonneville.utils.verify.valid_id(self.opts, id_):
            return False
        pub_path = os.path.join(self.opts['pki_dir'], 'minions', id_)
        with bonneville.utils.fopen(pub_path, 'r') as fp_:
            minion_pub = fp_.read()
        tmp_pub = bonneville.utils.mkstemp()
        with bonneville.utils.fopen(tmp_pub, 'w+') as fp_:
    ...
    def _ext_nodes(self, load):
        ...
        if self.opts['external_nodes']:
            if not bonneville.utils.which(self.opts['external_nodes']):
    ...
    def _return(self, load):
        ...
        self.serial.dump(
            ...
            bonneville.utils.atomicfile.atomic_open(
    ...
    def _syndic_return(self, load):
        ...
        if not os.path.isdir(jid_dir):
            os.makedirs(jid_dir)
            if 'load' in load:
                with bonneville.utils.fopen(os.path.join(jid_dir, '.load.p'), 'w+b') as fp_:
                    self.serial.dump(load['load'], fp_)
        wtag = os.path.join(jid_dir, 'wtag_{0}'.format(load['id']))
        try:
            with bonneville.utils.fopen(wtag, 'w+b') as fp_:
                fp_.write('')
    ...
    def minion_runner(self, clear_load):
        ...
        runner = bonneville.runner.Runner(opts)
        return runner.run()
    ...
    def pub_ret(self, load):
        ...
        with bonneville.utils.fopen(jid_fn, 'r') as fp_:
    ...
    def minion_pub(self, clear_load):
        ...
        with bonneville.utils.fopen(jid_fn, 'w+') as fp_:
    ...
    def revoke_auth(self, load):
        ...
        if not self.__verify_minion(load['id'], load['tok']):
            ...
            return False
        keyapi = bonneville.key.Key(self.opts)
    ...
    def run_func(self, func, load):
        ...
        if func == '_pillar' and 'id' in load:
            ...
            pubfn = os.path.join(self.opts['pki_dir'],
                    ...
                    load['id'])
            key = bonneville.crypt.Crypticle.generate_key_string()
            pcrypt = bonneville.crypt.Crypticle(
                    self.opts,
                    key)
            try:
...
class ClearFuncs(object):
    ...
    def __init__(self, opts, key, master_key, crypticle):
        ...
        self.ckminions = bonneville.utils.minions.CkMinions(opts)
        # Make an Auth object
        self.loadauth = bonneville.auth.LoadAuth(opts)
        ...
        self.mminion = bonneville.minion.MasterMinion(
                ...
                rend=False)
        # Make a wheel object
        self.wheel_ = bonneville.wheel.Wheel(opts)
    ...
    def _cluster_load(self):
        ...
        minions = {}
        master_pem = ''
        with bonneville.utils.fopen(self.opts['conf_file'], 'r') as fp_:
        ...
        for host in os.listdir(minion_dir):
            ...
            minions[host] = bonneville.utils.fopen(pub, 'r').read()
        if self.opts['cluster_mode'] == 'full':
            master_pem_path = os.path.join(self.opts['pki_dir'], 'master.pem')
            with bonneville.utils.fopen(master_pem_path) as fp_:
                master_pem = fp_.read()
        return [minions,
                master_conf,
                master_pem,
    ...
    def _check_permissions(self, filename):
        ...
        '''
        if bonneville.utils.is_windows():
            return True
    ...
    def _check_autosign(self, keyid):
        ...
        with bonneville.utils.fopen(autosign_file, 'r') as fp_:
            for line in fp_:
                line = line.strip()
    ...
    def _auth(self, load):
        ...
        # Encrypt the AES key as an encrypted bonneville.payload
        ...
        if self.opts['open_mode']:
            ...
            if bonneville.utils.fopen(pubfn, 'r').read() != load['pub']:
                log.error(
            ...
            with bonneville.utils.fopen(pubfn_pend, 'w+') as fp_:
                fp_.write(load['pub'])
            ret = {'enc': 'clear',
                   'load': {'ret': True}}
            eload = {'result': True,
            ...
            # ret False
            if bonneville.utils.fopen(pubfn_pend, 'r').read() != load['pub']:
                log.error(
                    'Authentication attempt from {id} failed, the public '
        ...
        if not os.path.isfile(pubfn) or self.opts['open_mode']:
            with bonneville.utils.fopen(pubfn, 'w+') as fp_:
                fp_.write(load['pub'])
        pub = None
    ...
    def runner(self, clear_load):
        ...
        if 'token' in clear_load:
            ...
            try:
                ...
                runner_client = bonneville.runner.RunnerClient(self.opts)
    ...
    def publish(self, clear_load):
        ...
        if not clear_load['jid']:
            clear_load['jid'] = bonneville.utils.prep_jid(
                    self.opts['cachedir'],
                    self.opts['hash_type'],
        ...
        self.serial.dump(
                clear_load,
                bonneville.utils.fopen(os.path.join(jid_dir, '.load.p'), 'w+b')
                )
        ...
        if self.opts['sign_pub_messages']:
            ...
            payload['sig'] = bonneville.crypt.sign_message(master_pem_path, payload['load'])
