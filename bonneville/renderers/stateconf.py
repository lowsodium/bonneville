...
:platform: all
'''
# See http://docs.saltstack.org/en/latest/ref/renderers/all/bonneville.renderers.stateconf.html
...
def render(input, env='', sls='', argline='', **kws):
    ...
    def process_high_data(high, extract):
        ...
        try:
            ...
            from bonneville.state import State
    ...
    if ('-p', '') in opts:
        ...
        if isinstance(input, basestring):
            with bonneville.utils.fopen(input, 'r') as ifile:
...
def rename_state_ids(data, sls, is_extend=False):
    ...
    # "my.bonneville.file::" + the_state_id_without_the_first_dot.
...
def add_start_state(data, sls):
    ...
    reqin = {state_name(data[sid].keys().next()): sid}
