...
import bonneville.cli.caller
import bonneville.cli.cp
import bonneville.cli.batch
import bonneville.client
import bonneville.client.ssh
import bonneville.output
import bonneville.runner
import bonneville.auth
import bonneville.key

from bonneville.utils import parsers
from bonneville.utils.verify import check_user, verify_env, verify_files
...
class SaltCMD(parsers.SaltCMDOptionParser):
    ...
    def run(self):
        ...
        try:
            local = bonneville.client.LocalClient(self.get_config_file_path())
        except SaltClientError as exc:
            self.exit(2, '{0}\n'.format(exc))
            return

        if self.options.batch:
            batch = bonneville.cli.batch.Batch(self.config)
            ...
            if not 'token' in kwargs and self.options.eauth:
                resolver = bonneville.auth.Resolver(self.config)
    ...
    def _output_ret(self, ret, out):
        ...
        if self.config['fun'] == 'sys.doc' and not isinstance(ret, Exception):
            ...
            bonneville.output.display_output(ret, out, self.config)
        if not ret:
            sys.exit(2)
...
class SaltCP(parsers.SaltCPOptionParser):
    ...
    def run(self):
        ...
        cp_ = bonneville.cli.cp.SaltCP(self.config)
        cp_.run()
...
class SaltKey(parsers.SaltKeyOptionParser):
    ...
    def run(self):
        ...
        key = bonneville.key.KeyCLI(self.config)
...
class SaltCall(parsers.SaltCallOptionParser):
    ...
    def run(self):
        ...
        caller = bonneville.cli.caller.Caller(self.config)
...
class SaltRun(parsers.SaltRunOptionParser):
    ...
    def run(self):
        ...
        runner = bonneville.runner.Runner(self.config)
        if self.options.doc:
            runner._print_docs()
            self.exit(0)
...
class SaltSSH(parsers.SaltSSHOptionParser):
    ...
    def run(self):
        ...
        ssh = bonneville.client.ssh.SSH(self.config)
        ssh.run()
