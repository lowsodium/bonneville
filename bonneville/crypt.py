...
def dropfile(cachedir, user=None):
    ...
    mask = os.umask(0o277)
    with bonneville.utils.fopen(dfnt, 'w+') as fp_:
        fp_.write(aes)
    if user:
        try:
            import pwd
...
def sign_message(privkey_path, message):
    ...
    '''
    log.debug('bonneville.crypt.sign_message: Loading private key')
    evp_rsa = EVP.load_key(privkey_path)
    evp_rsa.sign_init()
    evp_rsa.sign_update(message)
    log.debug('bonneville.crypt.sign_message: Signing message.')
...
def verify_signature(pubkey_path, message, signature):
    ...
    log.debug('bonneville.crypt.verify_signature: Loading public key')
    pubkey = RSA.load_pub_key(pubkey_path)
    verify_evp = EVP.PKey()
    verify_evp.assign_rsa(pubkey)
    verify_evp.verify_init()
    verify_evp.verify_update(message)
    log.debug('bonneville.crypt.verify_signature: Verifying signature')
...
class MasterKeys(dict):
    ...
    def get_pub_str(self):
        ...
        return bonneville.utils.fopen(self.pub_path, 'r').read()
...
class Auth(object):
    ...
    def get_keys(self):
        ...
        bonneville.utils.verify.check_path_traversal(self.opts['pki_dir'], user)
    ...
    def minion_sign_in_payload(self):
        ...
        try:
            ...
            pass
        with bonneville.utils.fopen(tmp_pub, 'r') as fp_:
    ...
    def verify_master(self, payload):
        ...
        if os.path.isfile(m_pub_fn) and not self.opts['open_mode']:
            local_master_pub = bonneville.utils.fopen(m_pub_fn).read()
            ...
            bonneville.utils.fopen(m_pub_fn, 'w+').write(payload['pub_key'])
    ...
    def sign_in(self, timeout=60, safe=True):
        ...
        m_pub_fn = os.path.join(self.opts['pki_dir'], self.mpub)
        try:
            self.opts['master_ip'] = bonneville.utils.dns_check(
                self.opts['master'],
                True,
                self.opts['ipv6']
            )
        ...
        if not auth['aes']:
            log.critical(
                ...
                'at:\n{1}'.format(bonneville.version.__version__, m_pub_fn)
            )
            sys.exit(42)
        if self.opts.get('master_finger', False):
            if bonneville.utils.pem_finger(m_pub_fn) != self.opts['master_finger']:
                log.critical(
                    ...
                    .format(
                        ...
                        bonneville.utils.pem_finger(m_pub_fn)
                    )
                )
                sys.exit(42)
