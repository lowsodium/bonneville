...
def check_env_cache(opts, env_cache):
    ...
    if not os.path.isfile(env_cache):
        return None
    try:
        with bonneville.utils.fopen(env_cache, 'rb') as fp_:
            log.trace('Returning env cache data from {0}'.format(env_cache))
            serial = bonneville.payload.Serial(opts)
...
def generate_mtime_map(path_map):
    ...
    '''
    file_map = {}
    for env, path_list in path_map.items():
...
class Fileserver(object):
    ...
    def __init__(self, opts):
        self.opts = opts
        self.servers = bonneville.loader.fileserver(opts, opts['fileserver_backend'])
