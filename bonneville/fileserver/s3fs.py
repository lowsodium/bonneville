...
import bonneville.fileserver as fs
import bonneville.modules
import bonneville.utils
import bonneville.utils.s3 as s3
...
def update():
    ...
    if _s3_sync_on_update:
        ...
        for env, env_meta in metadata.items():
...
def find_file(path, env='base', **kwargs):
    ...
    for bucket_name, files in env_files.items():
...
def file_hash(load, fnd):
    ...
    if os.path.isfile(cached_file_path):
        ret['hsum'] = bonneville.utils.get_hash(cached_file_path)
        ret['hash_type'] = 'md5'
...
def serve_file(load, fnd):
    ...
    with bonneville.utils.fopen(cached_file_path, 'rb') as fp_:
        fp_.seek(load['loc'])
...
def _refresh_buckets_cache_file(cache_file):
    ...
    with bonneville.utils.fopen(cache_file, 'w') as fp_:
...
def _read_buckets_cache_file(cache_file):
    ...
    with bonneville.utils.fopen(cache_file, 'rb') as fp_:
...
def _find_files(metadata, dirs_only=False):
    ...
    for bucket_name, data in metadata.items():
