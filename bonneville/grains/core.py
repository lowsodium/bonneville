...
import bonneville.modules.cmdmod

__salt__ = {
    'cmd.run': bonneville.modules.cmdmod._run_quiet,
    'cmd.run_all': bonneville.modules.cmdmod._run_all_quiet
}
...
if bonneville.utils.is_windows():
    ...
    try:
        import wmi
        import bonneville.utils.winapi
        HAS_WMI = True
    except ImportError:
        log.exception(
...
def _linux_cpudata():
    ...
    if os.path.isfile(cpuinfo):
        with bonneville.utils.fopen(cpuinfo, 'r') as _fp:
            for line in _fp:
                comps = line.split(':')
                if not len(comps) > 1:
                    continue
...
def _linux_gpu_data():
    ...
    gpus:
      - vendor: nvidia|amd|ati|...
        model: string
    '''
    lspci = bonneville.utils.which('lspci')
    if not lspci:
        log.info(
...
def _bsd_cpudata(osdata):
    ...
    #   cpu_model
    #   cpu_flags
    sysctl = bonneville.utils.which('sysctl')
    arch = bonneville.utils.which('arch')
    cmds = {}
    ...
    if osdata['kernel'] == 'FreeBSD' and os.path.isfile('/var/run/dmesg.boot'):
        ...
        with bonneville.utils.fopen('/var/run/dmesg.boot', 'r') as _fp:
            cpu_here = False
            for line in _fp:
...
def _memdata(osdata):
    ...
    if osdata['kernel'] == 'Linux':
        ...
        if os.path.isfile(meminfo):
            with bonneville.utils.fopen(meminfo, 'r') as ifile:
                for line in ifile:
                    comps = line.rstrip('\n').split(':')
                    if not len(comps) > 1:
                        continue
        ...
        with bonneville.utils.winapi.Com():
            wmi_c = wmi.WMI()
...
def _virtual(osdata):
    ...
    for command in ('dmidecode', 'lspci', 'dmesg'):
        ...
        cmd = bonneville.utils.which(command)
        ...
        if ret['retcode'] > 0:
            if bonneville.log.is_logging_configured():
                if bonneville.utils.is_windows():
                    continue
                log.warn(
    ...
    if osdata['kernel'] in choices:
        if isdir('/proc/vz'):
            ...
            if os.path.isfile('/proc/xen/xsd_kva'):
                ...
                if grains.get('productname', '') == 'HVM domU':
                    ...
                    caps = bonneville.utils.fopen('/proc/xen/capabilities')
        ...
        if os.path.isfile('/proc/cpuinfo'):
            if 'QEMU Virtual CPU' in bonneville.utils.fopen('/proc/cpuinfo', 'r').read():
                grains['virtual'] = 'kvm'
    elif osdata['kernel'] == 'FreeBSD':
        kenv = bonneville.utils.which('kenv')
        if kenv:
        ...
        zonename = bonneville.utils.which('zonename')
        if zonename:
...
def os_data():
    ...
    if bonneville.utils.is_windows():
        ...
        return grains
    elif bonneville.utils.is_linux():
        ...
        try:
            ...
            if os.path.isfile('/etc/lsb-release'):
                with bonneville.utils.fopen('/etc/lsb-release') as ifile:
                    for line in ifile:
                ...
                # Arch ARM Linux
                with bonneville.utils.fopen('/etc/os-release') as ifile:
                    # Imitate lsb-release
                    for line in ifile:
                        # NAME="Arch Linux ARM"
                        # ID=archarm
                ...
                with bonneville.utils.fopen('/etc/altlinux-release') as ifile:
                ...
                with bonneville.utils.fopen('/etc/centos-release') as ifile:
                    for line in ifile:
        ...
        # so that distro.linux_distribution() does the /etc/lsb-release
        ...
        if 'joyent_' in uname_v:
            ...
            with bonneville.utils.fopen('/etc/release', 'r') as fp_:
                rel_data = fp_.read()
                try:
...
def ip4():
    ...
    '''
    return {'ipv4': bonneville.utils.network.ip_addrs(include_loopback=True)}
...
def ip6():
    ...
    '''
    return {'ipv6': bonneville.utils.network.ip_addrs6(include_loopback=True)}
...
def ip_interfaces():
    ...
    #   ip_interfaces
    ret = {}
    ifaces = bonneville.utils.network.interfaces()
    for face in ifaces:
        iface_ips = []
...
def saltversion():
    ...
    # Provides:
    #   saltversion
    from bonneville.version import __version__
...
def saltversioninfo():
    ...
    # Provides:
    #   saltversioninfo
    from bonneville.version import __version_info__
...
def _dmidecode_data(regex_dict):
    ...
    if bonneville.utils.which('dmidecode'):
        out = __salt__['cmd.run']('dmidecode')
    elif bonneville.utils.which('smbios'):
...
def _smartos_zone_data():
    ...
    if os.path.isfile('/etc/pkgsrc_version'):
        with bonneville.utils.fopen('/etc/pkgsrc_version', 'r') as fp_:
            for line in fp_:
                match = pkgsrcversion.match(line)
                if match:
                    grains['pkgsrcversion'] = match.group(1)
    if os.path.isfile('/etc/product'):
        with bonneville.utils.fopen('/etc/product', 'r') as fp_:
            for line in fp_:
