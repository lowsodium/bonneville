...
from bonneville.utils.decorators import memoize as real_memoize
...
def which(exe=None):
    ...
    if exe:
        ...
        if not is_windows():
            ...
            # searched, which lets bonneville.utils.which() work well when invoked by
        ...
        log.error(
            'No executable was passed to be searched by bonneville.utils.which()'
        )
    return None
...
def jid_load(jid, cachedir, sum_type, serial='msgpack'):
    ...
    if not os.path.isfile(load_fn):
        return {}
    serial = bonneville.payload.Serial(serial)
...
def check_or_die(command):
    ...
    Lazily import `bonneville.modules.cmdmod` to avoid any sort of circular
    dependencies.
    '''
    if command is None:
...
def test_mode(**kwargs):
    ...
    etc) contains a True value (as determined by bonneville.utils.is_true).
    '''
...
def valid_url(url, protos):
    ...
    '''
    if bonneville._compat.urlparse(url).scheme in protos:
        return True
    return False
...
def date_cast(date):
    ...
    # fuzzy date
    try:
        if isinstance(date, bonneville._compat.string_types):
            try:
                if HAS_TIMELIB:
...
def warn_until(version_info,
    ...
    if _version_info_ is None:
        _version_info_ = bonneville.version.__version_info__
...
def memoize(func):
    '''
    Deprecation warning wrapper since memoize is now on bonneville.utils.decorators
    '''
    warn_until(
        (0, 19),
        'The \'memoize\' decorator was moved to \'bonneville.utils.decorators\', '
