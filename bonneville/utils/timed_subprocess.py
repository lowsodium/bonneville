    ...
    def wait(self, timeout=None):
        ...
        if timeout:
            if not isinstance(timeout, (int, float)):
                raise bonneville.exceptions.TimedProcTimeoutError('Error: timeout must be a number')
            ...
            if rt.isAlive():
                ...
                raise bonneville.exceptions.TimedProcTimeoutError('%s : Timed out after %s seconds' % (
                    self.command,
                    str(timeout),
                ))
        else:
