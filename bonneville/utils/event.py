...
For example:
Namspaced tag
    'bonneville.runner.manage.status.start'
...
class SaltEvent(object):
    ...
    def __init__(self, node, sock_dir=None, **kwargs):
        self.serial = bonneville.payload.Serial({'serial': 'msgpack'})
    ...
    def __load_uri(self, sock_dir, node, **kwargs):
        ...
        if node == 'master':
            ...
            bonneville.utils.check_ipc_path_max_len(puburi)
            pulluri = 'ipc://{0}'.format(os.path.join(
                    sock_dir,
                    'master_event_pull.ipc'
                    ))
            bonneville.utils.check_ipc_path_max_len(pulluri)
        else:
...
class Reactor(multiprocessing.Process, bonneville.state.Compiler):
    '''
    ...
    def __init__(self, opts):
        ...
        bonneville.state.Compiler.__init__(self, opts)
    ...
    def list_reactors(self, tag):
        ...
        if isinstance(self.opts['reactor'], basestring):
            try:
                with bonneville.utils.fopen(self.opts['reactor']) as fp_:
...
class ReactWrap(object):
    ...
    def cmd(self, *args, **kwargs):
        '''
        Wrap LocalClient for running :ref:`execution modules <all-bonneville.modules>`
        '''
        local = bonneville.client.LocalClient(self.opts['conf_file'])
        return local.cmd_async(*args, **kwargs)

    def runner(self, fun, **kwargs):
        '''
        Wrap RunnerClient for executing :ref:`runner modules <all-bonneville.runners>`
        '''
        runner = bonneville.runner.RunnerClient(self.opts)
        return runner.low(fun, kwargs)

    def wheel(self, fun, **kwargs):
        '''
        Wrap Wheel to enable executing :ref:`wheel modules <all-bonneville.wheel>`
        '''
        wheel = bonneville.wheel.Wheel(self.opts)
