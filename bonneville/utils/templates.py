...
from bonneville.exceptions import SaltRenderError
from bonneville.utils.jinja import SaltCacheLoader as JinjaSaltCacheLoader
from bonneville.utils.jinja import SerializerExtension as JinjaSerializerExtension
from bonneville import __path__ as saltpath
        ...
        if isinstance(tmplsrc, basestring):
            if from_str:
                ...
                try:
                    ...
                    if bonneville.utils.is_bin_file(tmplsrc):
...
def render_jinja_tmpl(tmplstr, context, tmplpath=None):
    ...
    jinja_env.filters['strftime'] = bonneville.utils.date_format
...
def render_mako_tmpl(tmplstr, context, tmplpath=None):
    ...
    from bonneville.utils.mako import SaltMakoTemplateLookup
...
def py(sfn, string=False, **kwargs):  # pylint: disable=C0103
    ...
    try:
        ...
        if string:
            return {'result': True,
                    'data': data}
        tgt = bonneville.utils.mkstemp()
        with bonneville.utils.fopen(tgt, 'w+') as target:
            target.write(data)
        return {'result': True,
                'data': tgt}
    except Exception:
