...
from bonneville.exceptions import (
...
import bonneville.utils.schedule
from bonneville._compat import string_types
from bonneville.utils.debug import enable_sigusr1_handler
...
def resolve_dns(opts):
    ...
    if check_dns is True:
        # Because I import bonneville.log below I need to re-import bonneville.utils here
        import bonneville.utils
        try:
            ret['master_ip'] = \
                    bonneville.utils.dns_check(opts['master'], True, opts['ipv6'])
            ...
            if opts['retry_dns']:
                while True:
                    ...
                    if bonneville.log.is_console_configured():
                        log.warn(msg)
                    else:
                        print('WARNING: {0}'.format(msg))
                    time.sleep(opts['retry_dns'])
                    try:
                        ret['master_ip'] = bonneville.utils.dns_check(
...
def parse_args_and_kwargs(func, args, data=None):
    ...
    dictionaries.
    '''
    argspec = bonneville.utils.get_function_argspec(func)
    _args = []
    kwargs = {}
    invalid_kwargs = []
    ...
    for arg in args:
        ...
        if isinstance(arg, string_types):
            arg_name, arg_value = bonneville.utils.parse_kwarg(arg)
            if arg_name:
...
class SMinion(object):
    ...
    def __init__(self, opts):
        ...
        # module
        opts['grains'] = bonneville.loader.grains(opts)
        self.opts = opts
    ...
    def gen_modules(self):
        ...
        '''
        self.opts['pillar'] = bonneville.pillar.get_pillar(
            self.opts,
            self.opts['grains'],
            self.opts['id'],
            self.opts['environment'],
        ).compile_pillar()
        self.functions = bonneville.loader.minion_mods(self.opts)
        self.returners = bonneville.loader.returners(self.opts, self.functions)
        self.states = bonneville.loader.states(self.opts, self.functions)
        self.rend = bonneville.loader.render(self.opts, self.functions)
...
class MasterMinion(object):
    ...
    def __init__(
            ...
            matcher=True,
            whitelist=None):
        self.opts = bonneville.config.minion_config(opts['conf_file'])
        self.opts.update(opts)
        self.whitelist = whitelist
        self.opts['grains'] = bonneville.loader.grains(opts)
        self.opts['pillar'] = {}
    ...
    def gen_modules(self):
        ...
        Load all of the modules for the minion
        '''
        self.functions = bonneville.loader.minion_mods(
            self.opts,
...
class MultiMinion(object):
    ...
    def tune_in(self):
        ...
        if self.opts.get('ipc_mode', '') == 'tcp':
            ...
            bonneville.utils.check_ipc_path_max_len(epub_uri)
            epull_uri = 'ipc://{0}'.format(epull_sock_path)
            bonneville.utils.check_ipc_path_max_len(epull_uri)
        log.debug(
...
class Minion(object):
    ...
    def __init__(self, opts, timeout=60, safe=True):
        ...
        self.serial = bonneville.payload.Serial(self.opts)
        self.mod_opts = self.__prep_mod_opts()
        self.functions, self.returners = self.__load_modules()
        self.matcher = Matcher(self.opts, self.functions)
        self.proc_dir = get_proc_dir(opts['cachedir'])
        self.schedule = bonneville.utils.schedule.Schedule(
            self.opts,
            self.functions,
            self.returners)
    ...
    def __load_modules(self):
        ...
        module
        '''
        self.opts['grains'] = bonneville.loader.grains(self.opts)
        functions = bonneville.loader.minion_mods(self.opts)
        returners = bonneville.loader.returners(self.opts, functions)
    ...
    def _fire_master(self, data=None, tag=None, events=None, pretag=None):
        ...
        if events:
            ...
            return
        sreq = bonneville.payload.SREQ(self.opts['master_uri'])
        try:
    ...
    def _handle_aes(self, load, sig=None):
        ...
        if self.functions['config.get']('sign_pub_messages') and not sig:
            bonneville.utils.warn_until((0, 17, 6), 'Master pub message signing is enabled but we '
        ...
        if not self.functions['config.get']('sign_pub_messages') and not sig:
            bonneville.utils.warn_until((0, 17, 6), 'Master pub message signing is disabled but we '
        ...
        if sig and self.functions['config.get']('sign_pub_messages'):
            if not bonneville.crypt.verify_signature(master_pubkey_path, load, sig):
    ...
    def _thread_return(cls, minion_instance, opts, data):
        ...
        if opts['multiprocessing']:
            ...
            bonneville.utils.daemonize_if(opts)
            sdata = {'pid': os.getpid()}
            sdata.update(data)
            with bonneville.utils.fopen(fn_, 'w+b') as fp_:
        ...
        if function_name in minion_instance.functions:
            try:
                ...
                aspec = bonneville.utils.get_function_argspec(
    ...
    def _return_pub(self, ret, ret_cmd='_return'):
        ...
        if self.opts['cache_jobs']:
            ...
            if not os.path.isdir(jdir):
                os.makedirs(jdir)
            bonneville.utils.fopen(fn_, 'w+b').write(self.serial.dumps(ret))
        return ret_val
    ...
    def authenticate(self, timeout=60, safe=True):
        ...
        auth = bonneville.crypt.Auth(self.opts)
        ...
        self.crypticle = bonneville.crypt.Crypticle(self.opts, self.aes)
    ...
    def tune_in(self):
        ...
        try:
            ...
            log.log(
                bonneville.utils.is_windows() and logging.DEBUG or logging.ERROR,
        ...
        bonneville.utils.enable_ctrl_logoff_handler()
...
class Syndic(Minion):
    ...
    def tune_in(self):
        ...
        self.local = bonneville.client.LocalClient(self.opts['_minion_conf_file'])
            ...
            try:
                ...
                while True:
                    ...
                    if event is None:
                        # Timeout reached
                        break
                    if bonneville.utils.is_jid(event['tag']) and 'return' in event['data']:
                        if not event['tag'] in jids:
                            ...
                            jids[event['tag']]['__load__'] = bonneville.utils.jid_load(
                                event['data']['jid'],
...
class Matcher(object):
    ...
    def grain_match(self, tgt, delim=':'):
        ...
        if delim not in tgt:
            ...
            return False
        return bonneville.utils.subdict_match(self.opts['grains'], tgt, delim=delim)
    ...
    def grain_pcre_match(self, tgt, delim=':'):
        ...
        if delim not in tgt:
            log.error('Got insufficient arguments for grains pcre match '
                      'statement from master')
            return False
        return bonneville.utils.subdict_match(self.opts['grains'], tgt,
    ...
    def pillar_match(self, tgt, delim=':'):
        ...
        if delim not in tgt:
            log.error('Got insufficient arguments for pillar match '
                      'statement from master')
            return False
        return bonneville.utils.subdict_match(self.opts['pillar'], tgt, delim=delim)
    ...
    def ipcidr_match(self, tgt):
        ...
        if num_parts > 2:
            ...
            # Target is CIDR
            return bonneville.utils.network.in_subnet(
                tgt,
    ...
    def nodegroup_match(self, tgt, nodegroups):
        ...
        if tgt in nodegroups:
            return self.compound_match(
                bonneville.utils.minions.nodegroup_comp(tgt, nodegroups)
            )
        return False
