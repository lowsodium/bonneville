...
def _set_retcode(ret):
    ...
    if isinstance(ret, list):
        ...
        return
    if not bonneville.utils.check_state_result(ret):
...
def running():
    ...
    for data in active:
        err = (
            ...
            data['fun'],
            data['pid'],
            bonneville.utils.jid_to_time(data['jid']),
            data['jid'],
        )
        ret.append(err)
    return ret
...
def low(data, queue=False, **kwargs):
    ...
    if queue:
        ...
        if conflict:
            __context__['retcode'] = 1
            return conflict
    st_ = bonneville.state.State(__opts__)
    ...
    if isinstance(ret, list):
        __context__['retcode'] = 1
    if bonneville.utils.check_state_result(ret):
        __context__['retcode'] = 2
    return ret
...
def highstate(test=None, queue=False, **kwargs):
    ...
    if bonneville.utils.test_mode(test=test, **kwargs):
        opts['test'] = True
    else:
    ...
    st_ = bonneville.state.HighState(opts, pillar)
    st_.push_active()
    try:
    ...
    serial = bonneville.payload.Serial(__opts__)
    ...
    try:
        ...
        with bonneville.utils.fopen(cache_file, 'w+b') as fp_:
            serial.dump(ret, fp_)
...
def top(topfn, test=None, queue=False, **kwargs):
    ...
    if bonneville.utils.test_mode(test=test, **kwargs):
        ...
        opts['test'] = __opts__.get('test', None)
    st_ = bonneville.state.HighState(opts)
    st_.push_active()
...
def show_highstate(queue=False, **kwargs):
    ...
    if queue:
        ...
        if conflict:
            __context__['retcode'] = 1
            return conflict
    st_ = bonneville.state.HighState(__opts__)
    st_.push_active()
    try:
...
#    '''
#    st_ = bonneville.state.RemoteHighState(__opts__, __grains__)
...
def single(fun, name, test=None, queue=False, **kwargs):
    ...
    if bonneville.utils.test_mode(test=test, **kwargs):
        ...
        opts['test'] = __opts__.get('test', None)
    st_ = bonneville.state.State(opts)
...
def pkg(pkg_path, pkg_sum, hash_type, test=False, **kwargs):
    ...
    if not os.path.isfile(pkg_path):
        return {}
    if not bonneville.utils.get_hash(pkg_path, hash_type) == pkg_sum:
        return {}
    ...
    with bonneville.utils.fopen(lowstate_json, 'r') as fp_:
        lowstate = json.load(fp_, object_hook=bonneville.utils.decode_dict)
    ...
    st_ = bonneville.state.State(popts)
