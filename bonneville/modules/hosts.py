...
import bonneville.utils.odict as odict
...
def _list_hosts():
    ...
    if not os.path.isfile(hfn):
        return ret
    with bonneville.utils.fopen(hfn) as ifile:
        for line in ifile:
            line = line.strip()
            if not line:
                continue
...
def set_host(ip, alias):
    ...
    if not os.path.isfile(hfn):
        return False
    lines = bonneville.utils.fopen(hfn).readlines()
    ...
    if not ovr:
        ...
        line = ip + '\t\t' + alias + '\n'
        lines.append(line)
    with bonneville.utils.fopen(hfn, 'w+') as ofile:
