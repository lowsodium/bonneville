...
import bonneville.utils.validate.net
from bonneville.modules import __path__ as saltmodpath
...
def _parse_settings_eth(opts, iface_type, enabled, iface):
    ...
    if iface_type not in ['bond', 'vlan', 'bridge']:
        if 'addr' in opts:
            if bonneville.utils.validate.net.mac(opts['addr']):
...
def _parse_network_settings(opts, current):
    ...
    # Normalize keys
    opts = dict((k.lower(), v) for (k, v) in opts.items())
    current = dict((k.lower(), v) for (k, v) in current.items())
    result = {}
...
def _read_file(path):
    ...
    '''
    try:
        with bonneville.utils.fopen(path, 'rb') as contents:
...
def _write_file_iface(iface, data, folder, pattern):
    ...
    if not os.path.exists(folder):
        ...
        log.error(msg)
        raise AttributeError(msg)
    fout = bonneville.utils.fopen(filename, 'w')
    fout.write(data)
    fout.close()
