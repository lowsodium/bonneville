...
def _get_ppa_info_from_launchpad(owner_name, ppa_name):
    ...
    Uses urllib.request which sacrifices server cert verification.
    ...
    lp_url = 'https://launchpad.net/api/1.0/~{0}/+archive/{1}'.format(
        owner_name, ppa_name)
    request = urllib.request.Request(lp_url, headers={'Accept': 'application/json'})
...
def latest_version(*names, **kwargs):
    ...
    '''
    refresh = bonneville.utils.is_true(kwargs.pop('refresh', True))
    ...
    for name in names:
        ...
        if not installed:
            ...
            if not any(
                (bonneville.utils.compare_versions(ver1=x,
                                             oper='>=',
                                             ver2=candidate,
...
def install(name=None,
    ...
    Returns a dict containing the new package names and versions::
        ...
        {'<package>': {'old': '<old-version>',
                       'new': '<new-version>'}}
    '''
    if bonneville.utils.is_true(refresh):
        refresh_db()
    ...
    if pkg_params is None or len(pkg_params) == 0:
        ...
        for param, version_num in pkg_params.items():
            if version_num is None:
                ...
                cver = old.get(param)
                if cver is not None \
                        and bonneville.utils.compare_versions(ver1=version_num,
                                                        oper='<',
                                                        ver2=cver,
                                                        cmp_func=version_cmp):
                    downgrade = True
...
def list_pkgs(versions_as_list=False, removed=False):
    ...
    '''
    versions_as_list = bonneville.utils.is_true(versions_as_list)
    removed = bonneville.utils.is_true(removed)
    ...
    if not removed and __salt__['cmd.has_exec']('grep-available'):
        ...
        virtpkgs = set()
        for realpkg, provides in virtpkgs_all.items():
...
def del_repo(repo, **kwargs):
    ...
    if repos:
        ...
        if deleted_from:
            ...
            for repo_file, c in deleted_from.items():
...
def set_selections(path=None, selection=None, clear=False):
    ...
    if path:
        ...
        with bonneville.utils.fopen(path, 'r') as ifile:
