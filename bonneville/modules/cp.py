...
def _auth():
    ...
    if not 'auth' in __context__:
        __context__['auth'] = bonneville.crypt.SAuth(__opts__)
...
def recv(files, dest):
    ...
    for path, data in files.items():
        ...
        try:
            bonneville.utils.fopen(final, 'w+').write(data)
            ret[final] = True
        except IOError:
            ret[final] = False
...
def _mk_client():
    ...
    if not 'cp.fileclient' in __context__:
        __context__['cp.fileclient'] = \
                bonneville.fileclient.get_file_client(__opts__)
...
def get_file_str(path, env='base'):
    ...
    with bonneville.utils.fopen(fn_, 'r') as fp_:
        data = fp_.read()
    return data
...
def push(path):
    ...
    sreq = bonneville.payload.SREQ(__opts__['master_uri'])
