...
def __virtual__():
    ...
    '''
    if bonneville.utils.is_windows() and HAS_DEPENDENCIES:
        return 'pkg'
    return False
...
def latest_version(*names, **kwargs):
    ...
    if bonneville.utils.is_true(kwargs.get('refresh', True)):
        refresh_db()
    ...
    for name in names:
        ...
        if len(pkginfo) == 1:
            ...
            if bonneville.utils.compare_versions(ver1=str(candidate),
                                           oper='>',
                                           ver2=str(version_num)):
                ret[name] = candidate
            continue
        for ver in pkginfo.keys():
            if bonneville.utils.compare_versions(ver1=str(ver),
                                           oper='>',
                                           ver2=str(candidate)):
                candidate = ver
        full_name = pkginfo[candidate]['full_name']
...
def get_repo_data():
    ...
    try:
        with bonneville.utils.fopen(cached_repo, 'rb') as repofile:
            try:
