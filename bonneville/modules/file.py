...
import bonneville.utils.filebuffer
from bonneville.exceptions import CommandExecutionError, SaltInvocationError
import bonneville._compat
...
def _binary_replace(old, new):
    ...
    files differ.
    '''
    old_isbin = not bonneville.utils.istextfile(old)
    new_isbin = not bonneville.utils.istextfile(new)
...
def get_sum(path, form='md5'):
    ...
    if not os.path.isfile(path):
        return 'File not found'
    try:
        with bonneville.utils.fopen(path, 'rb') as ifile:
...
def get_hash(path, form='md5', chunk_size=4096):
    ...
    '''
    return bonneville.utils.get_hash(path, form, chunk_size)
...
def find(path, **kwargs):
    ...
    '''
    try:
        finder = bonneville.utils.find.Finder(kwargs)
...
def psed(path,
    ...
    ofile = bonneville.utils.fopen(path, 'w')
    with bonneville.utils.fopen('{0}{1}'.format(path, backup), 'r') as ifile:
        if multi is True:
...
def replace(path,
    ...
    if not bonneville.utils.istextfile(path):
        raise SaltInvocationError(
    ...
    if not bonneville.utils.is_windows():
    ...
    if not dry_run and not bonneville.utils.is_windows():
...
def search(path,
    ...
    Params are identical to :py:func:`~bonneville.modules.file.replace`.
...
def contains(path, text):
    ...
    try:
        with bonneville.utils.filebuffer.BufferedReader(path) as breader:
...
def contains_regex(path, regex, lchar=''):
    ...
    try:
        with bonneville.utils.fopen(path, 'r') as target:
            for line in target:
                if lchar:
...
def append(path, *args):
    ...
    .. code-block:: bash
        ...
        salt '*' file.append /etc/motd \\
                "With all thine offerings thou shalt offer bonneville." \\
                "Salt is what makes things taste bad when it isn't in them."
    '''
    # Largely inspired by Fabric's contrib.files.append()

    with bonneville.utils.fopen(path, "a") as ofile:
        for line in args:
            ofile.write('{0}\n'.format(line))
    ...
    if isinstance(source, list):
        ...
        for single in source:
            if isinstance(single, dict):
                ...
                proto = bonneville._compat.urlparse(single_src).scheme
                if proto == 'salt':
                    if single_src[7:] in mfiles or single_src[7:] in mdirs:
                        source = single_src
                        break
                elif proto.startswith('http') or proto == 'ftp':
                    dest = bonneville.utils.mkstemp()
                    ...
                    if fn_:
                        ...
                        break
            elif isinstance(single, bonneville._compat.string_types):
...
def get_managed(
    ...
    if template and source:
        ...
        if template in bonneville.utils.templates.TEMPLATE_REGISTRY:
            context_dict = defaults if defaults else {}
            if context:
                context_dict.update(context)
            data = bonneville.utils.templates.TEMPLATE_REGISTRY[template](
                sfn,
                name=name,
                source=source,
                user=user,
...
def check_file_meta(
                ...
                if __salt__['config.option']('obfuscate_templates'):
                    ...
                    if bdiff:
                        ...
                        with contextlib.nested(
                                bonneville.utils.fopen(sfn, 'rb'),
                                bonneville.utils.fopen(name, 'rb')) as (src, name_):
    ...
    if contents is not None:
        ...
        tmp = bonneville.utils.mkstemp(text=True)
        with bonneville.utils.fopen(tmp, 'w') as tmp_:
            tmp_.write(str(contents))
        # Compare the static contents with the named file
        with contextlib.nested(
                bonneville.utils.fopen(tmp, 'rb'),
                bonneville.utils.fopen(name, 'rb')) as (src, name_):
            slines = src.readlines()
            nlines = name_.readlines()
        if ''.join(nlines) != ''.join(slines):
            if __salt__['config.option']('obfuscate_templates'):
                ...
                if bonneville.utils.istextfile(name):
                    changes['diff'] = \
                        ''.join(difflib.unified_diff(nlines, slines))
                else:
...
def get_diff(
    ...
    if sfn:
        with contextlib.nested(bonneville.utils.fopen(sfn, 'r'),
                               bonneville.utils.fopen(minionfile, 'r')) \
                as (src, name_):
            slines = src.readlines()
            nlines = name_.readlines()
        if ''.join(nlines) != ''.join(slines):
...
def manage_file(name,
    ...
    if os.path.isfile(name):
        ...
        if mode is None and not bonneville.utils.is_windows():
            # Get current umask
            mask = os.umask(0)
            os.umask(mask)
...
def restore_backup(path, backup_id):
    ...
    backup_id
        ...
        :mod:`file.list_backups <bonneville.modules.file.list_backups>`
    ...
    bonneville.utils.backup_minion(path, _get_bkroot())
    try:
