...
'''
    ...
    for dev, args in nicp.items():
        ...
        if dmac in kwargs:
            ...
            nic_t = nic_t.replace('%%MAC%%', bonneville.utils.gen_mac())
        nic_str += nic_t
...
def init(name,
    ...
    bonneville.utils.copyfile(sfn, img_dest)
...
def create_xml_path(path):
    ...
    if not os.path.isfile(path):
        return False
    return create_xml_str(bonneville.utils.fopen(path, 'r').read())
...
def define_xml_path(path):
    ...
    if not os.path.isfile(path):
        return False
    return define_xml_str(bonneville.utils.fopen(path, 'r').read())
...
def define_vol_xml_path(path):
    ...
    if not os.path.isfile(path):
        return False
    return define_vol_xml_str(bonneville.utils.fopen(path, 'r').read())
...
def is_kvm_hyper():
    ...
    if __grains__['virtual'] != 'physical':
        return False
    try:
        if 'kvm_' not in bonneville.utils.fopen('/proc/modules').read():
            return False
    except IOError:
...
def is_xen_hyper():
    ...
    try:
        ...
        return False
    try:
        if 'xen_' not in bonneville.utils.fopen('/proc/modules').read():
            return False
    except IOError:
        # No /proc/modules? Are we on Windows? Or Solaris?
        return False
