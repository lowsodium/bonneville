...
def parse_hosts(hostsfile='/etc/hosts', hosts=None):
    ...
    if not hosts:
        try:
            with bonneville.utils.fopen(hostsfile, 'r') as fp_:
                hosts = fp_.read()
        except Exception:
...
def hosts_append(hostsfile='/etc/hosts', ip_addr=None, entries=None):
    ...
    with bonneville.utils.fopen(hostsfile, 'a') as fp_:
...
def hosts_remove(hostsfile='/etc/hosts', entries=None):
    ...
    host_list = entries.split(',')
    out_file = bonneville.utils.fopen(hostsfile, 'w')
    for line in hosts.splitlines():
...
def parse_zone(zonefile=None, zone=None):
    ...
    if zonefile:
        try:
            with bonneville.utils.fopen(zonefile, 'r') as fp_:
                zone = fp_.read()
        except Exception:
            pass
...
def _has_dig():
    ...
    '''
    return bonneville.utils.which('dig') is not None
