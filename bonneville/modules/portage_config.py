        ...
        if os.path.isdir(old_path):
            for triplet in os.walk(old_path):
                for file_name in triplet[2]:
                    ...
                    with bonneville.utils.fopen(file_path) as fh_:
                        for line in fh_:
                            line = line.strip()
            ...
            with bonneville.utils.fopen(old_path) as fh_:
                for line in fh_:
                    line = line.strip()
                    if line and not line.startswith('#'):
    ...
    if file_name in SUPPORTED_CONFS:
        ...
        if os.path.exists(path):
            if os.path.isdir(path):
                ...
                os.mkdir(path, 0o755)
                with bonneville.utils.fopen(path + '.tmpbak') as fh_:
                    for line in fh_:
                        line = line.strip()
                        if line and not line.startswith('#'):
        ...
        for triplet in os.walk(path):
            for file_name in triplet[2]:
                ...
                if cp[0] == '/' or cp.split('/') > 2:
                    rearrange.extend(list(bonneville.utils.fopen(file_path)))
                    os.remove(file_path)
                else:
                    new_contents = ''
                    with bonneville.utils.fopen(file_path, 'r+') as file_handler:
...
def append_to_package_conf(conf, atom='', flags=None, string='', overwrite=False):
    ...
    if conf in SUPPORTED_CONFS:
        ...
        try:
            file_handler = bonneville.utils.fopen(complete_file_path, 'r+')
        except IOError:
            file_handler = bonneville.utils.fopen(complete_file_path, 'w+')
...
def get_flags_from_package_conf(conf, atom):
    ...
    if conf in SUPPORTED_CONFS:
        ...
        flags = []
        try:
            file_handler = bonneville.utils.fopen(package_file)
        except IOError:
            return []
        else:
            for line in file_handler:
