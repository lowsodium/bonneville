...
def ex_mod_init(low):
    ...
    .. seealso::
       ...
       :py:func:`bonneville.states.pkg.mod_init`

       ``ebuild.ex_mod_init`` uses ``portage_config.enforce_nice_config`` to do
       the lifting.
       :py:func:`bonneville.modules.portage_config.enforce_nice_config`
...
def latest_version(*names, **kwargs):
    ...
    for name in names:
        ...
        if avail:
            if not installed \
                    or bonneville.utils.compare_versions(ver1=installed,
                                                   oper='<',
                                                   ver2=avail,
...
def refresh_db():
    ...
    if 'makeconf.features_contains'in __salt__ and __salt__['makeconf.features_contains']('webrsync-gpg'):
        ...
        if bonneville.utils.which('emerge-delta-webrsync'):
