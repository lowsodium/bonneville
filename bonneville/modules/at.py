...
def __virtual__():
    ...
    '''
    if bonneville.utils.is_windows() or not bonneville.utils.which('at'):
        return False
    return 'at'
...
def _cmd(binary, *args):
    ...
    binary = bonneville.utils.which(binary)
    if binary:
...
def atq(tag=None):
    ...
    for line in output.splitlines():
        ...
        if tag:
            ...
            # salt-call. With the int(), it fails with salt-call but not bonneville.
...
def atrm(*args):
    ...
    if not bonneville.utils.which('at'):
...
def at(*args, **kwargs):  # pylint: disable=C0103
    ...
    # Shim to produce output similar to what __virtual__() should do
    # but __salt__ isn't available in __virtual__()
    binary = bonneville.utils.which('at')
    if not binary:
