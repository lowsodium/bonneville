...
def __virtual__():
    ...
    '''
    if bonneville.utils.is_windows() and HAS_DEPENDENCIES is True:
        return 'network'
    return False
...
def ping(host):
    ...
    '''
    cmd = 'ping -n 4 {0}'.format(bonneville.utils.network.sanitize_host(host))
...
def traceroute(host):
    ...
    '''
    ret = []
    cmd = 'tracert {0}'.format(bonneville.utils.network.sanitize_host(host))
    lines = __salt__['cmd.run'](cmd).splitlines()
    for line in lines:
        if not ' ' in line:
            continue
...
def nslookup(host):
    ...
    ret = []
    addresses = []
    cmd = 'nslookup {0}'.format(bonneville.utils.network.sanitize_host(host))
    lines = __salt__['cmd.run'](cmd).splitlines()
    for line in lines:
        if addresses:
...
def dig(host):
    ...
    '''
    cmd = 'dig {0}'.format(bonneville.utils.network.sanitize_host(host))
    return __salt__['cmd.run'](cmd)
...
def interfaces():
    ...
    '''
    return bonneville.utils.network.interfaces()
...
def hw_addr(iface):
    ...
    '''
    return bonneville.utils.network.hw_addr(iface)
...
def subnets():
    ...
    '''
    return bonneville.utils.network.subnets()
...
def in_subnet(cidr):
    ...
    '''
    return bonneville.utils.network.in_subnet(cidr)
...
def ip_addrs(interface=None, include_loopback=False):
    ...
    '''
    return bonneville.utils.network.ip_addrs(interface=interface,
...
def ip_addrs6(interface=None, include_loopback=False):
    ...
    '''
    return bonneville.utils.network.ip_addrs6(interface=interface,
                                        include_loopback=include_loopback)
