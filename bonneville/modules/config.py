...
def get(key, default=''):
    ...
    '''
    ret = bonneville.utils.traverse_dict(__opts__, key, '_|-')
    if ret != '_|-':
        return ret
    ret = bonneville.utils.traverse_dict(__grains__, key, '_|-')
    if ret != '_|-':
        return ret
    ret = bonneville.utils.traverse_dict(__pillar__, key, '_|-')
    if ret != '_|-':
        return ret
    ret = bonneville.utils.traverse_dict(__pillar__.get('master', {}), key, '_|-')
    if ret != '_|-':
        return ret
    return default
...
def gather_bootstrap_script(replace=False):
    ...
    with bonneville.utils.fopen(fn_, 'w+') as fp_:
        fp_.write(urllib.request.urlopen('http://bootstrap.saltstack.org').read())
    return fn_
