...
from bonneville.modules.mac_user import _osmajor, _dscl, _flush_dscl_cache


def __virtual__():
    global _osmajor, _dscl, _flush_dscl_cache
    _osmajor = bonneville.utils.namespaced_function(_osmajor, globals())
    if __grains__.get('kernel') != 'Darwin' or _osmajor() < 10.7:
        return False
    _dscl = bonneville.utils.namespaced_function(_dscl, globals())
    _flush_dscl_cache = bonneville.utils.namespaced_function(
...
def add(name, gid=None, **kwargs):
    ...
    if bonneville.utils.contains_whitespace(name):
