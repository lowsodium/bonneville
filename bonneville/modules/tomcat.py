...
def _auth(uri):
    ...
    try:
        ...
        try:
            user = bonneville.utils.option('tomcat-manager.user', '', __opts__,
                    __pillar__)
            password = bonneville.utils.option('tomcat-manager.passwd', '', __opts__,
                    __pillar__)
        except Exception:
            return False
...
def _wget(cmd, opts=None, url='http://localhost:8080/manager', timeout=180):
    ...
    url6 += '{0}'.format(cmd)
    if opts:
        url += '?{0}'.format(urllib.parse.urlencode(opts))
        url6 += '?{0}'.format(urllib.parse.urlencode(opts))
...
def deploy_war(war,
    ...
    war
        ...
        tomcat) or a path supported by the bonneville.modules.cp.get_file function
    context
    ...
    url : http://localhost:8080/manager
        the URL of the server manager webapp
    env : base
        the environment for WAR file in used by bonneville.modules.cp.get_file
        function
    timeout : 180
        timeout for HTTP request
    ...
    tfile = war
    if war[0] != '/':
        tfile = os.path.join(tempfile.gettempdir(), 'bonneville.' +
