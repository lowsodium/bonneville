...
from bonneville.exceptions import CommandExecutionError, CommandNotFoundError
...
def install(pkgs=None,
    ...
    if use_wheel:
        ...
        if not bonneville.utils.compare_versions(ver1=cur_version, oper='>=',
                                           ver2=min_version):
            log.error(
    ...
    if find_links:
        ...
        for link in find_links:
            if not (bonneville.utils.valid_url(link, VALID_PROTOS) or os.path.exists(link)):
                raise CommandExecutionError(
    ...
    if index_url:
        if not bonneville.utils.valid_url(index_url, VALID_PROTOS):
            raise CommandExecutionError(
    ...
    if extra_index_url:
        if not bonneville.utils.valid_url(extra_index_url, VALID_PROTOS):
            raise CommandExecutionError(
    ...
    if pre_releases:
        ...
        if bonneville.utils.compare_versions(ver1=pip_version, oper='>=', ver2='1.4'):
            cmd.append('--pre')
