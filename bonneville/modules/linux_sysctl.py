...
from bonneville.modules.systemd import _sd_booted
...
def __virtual__():
    ...
    if __grains__['kernel'] != 'Linux':
        return False
    global _sd_booted
    _sd_booted = bonneville.utils.namespaced_function(_sd_booted, globals())
    return 'sysctl'
...
def default_config():
    ...
    running systemd >= 207 will use ``/etc/sysctl.d/99-bonneville.conf``.
    ...
    if _sd_booted():
        for line in __salt__['cmd.run_stdout'](
            ...
            if line.startswith('systemd '):
                ...
                try:
                    if int(version) >= 207:
                        return '/etc/sysctl.d/99-bonneville.conf'
                except ValueError:
                    log.error(
...
def persist(name, value, config=None):
    ...
    :mod:`sysctl.default_config <bonneville.modules.linux_sysctl.default_config>`.
    ...
    if not os.path.isfile(config):
        try:
            with bonneville.utils.fopen(config, 'w+') as _fh:
    ...
    nlines = []
    try:
        with bonneville.utils.fopen(config, 'r') as _fh:
