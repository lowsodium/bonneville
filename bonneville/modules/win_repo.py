...
def genrepo():
    ...
    for root, dirs, files in os.walk(repo):
        for name in files:
            if name.endswith('.sls'):
                with bonneville.utils.fopen(os.path.join(root, name), 'r') as slsfile:
                    try:
    ...
    with bonneville.utils.fopen(os.path.join(repo, winrepo), 'w+b') as repo:
        repo.write(msgpack.dumps(ret))
    bonneville.output.display_output(ret, 'pprint', __opts__)
    return ret
