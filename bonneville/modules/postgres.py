...
def __virtual__():
    ...
    '''
    if all((bonneville.utils.which('psql'), HAS_ALL_IMPORTS)):
        return 'postgres'
    return False
...
def _run_psql(cmd, runas=None, password=None, host=None,
    ...
    if password is not None:
        pgpassfile = bonneville.utils.mkstemp(text=True)
        with bonneville.utils.fopen(pgpassfile, 'w') as fp_:
...
def _psql_cmd(*args, **kwargs):
    ...
    cmd = [bonneville.utils.which('psql'),
           '--no-align',
           '--no-readline']
    if password is None:
        cmd += ['--no-password']
