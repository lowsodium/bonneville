...
from bonneville.utils import which as _which, which_bin as _which_bin
from bonneville.exceptions import SaltInvocationError
import bonneville.utils.decorators as decorators
...
def tar(options, tarfile, sources=None, dest=None, cwd=None, template=None):
    ...
    .. code-block:: bash
        ...
        salt '*' archive.tar template=jinja cjvf /tmp/bonneville.tar.bz2 {{grains.saltpath}}
    ...
    if isinstance(sources, bonneville._compat.string_types):
...
def unzip(zipfile, dest, excludes=None, template=None):
    ...
    '''
    if isinstance(excludes, bonneville._compat.string_types):
