...
def _replace_auth_key(
    ...
    try:
        ...
        with bonneville.utils.fopen(full, 'r') as _fh:
            for line in _fh:
                if line.startswith('#'):
                    # Commented Line
                    lines.append(line)
            ...
            with bonneville.utils.fopen(full, 'w') as _fh:
...
def _validate_keys(key_file):
    ...
    try:
        with bonneville.utils.fopen(key_file, 'r') as _fh:
            for line in _fh:
                if line.startswith('#'):
                    # Commented Line
                    continue
...
def host_keys(keydir=None):
    ...
    for fn_ in os.listdir(keydir):
        if fn_.startswith('ssh_host_'):
            ...
            try:
                with bonneville.utils.fopen(os.path.join(keydir, fn_), 'r') as _fh:
...
def set_auth_key(
    ...
    if status == 'update':
        ...
        try:
            with bonneville.utils.fopen(fconfig, 'a+') as _fh:
...
def set_known_host(user, hostname,
    ...
    try:
        with bonneville.utils.fopen(full, 'a') as ofile:
            ofile.write(line)
