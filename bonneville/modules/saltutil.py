...
from bonneville.exceptions import SaltReqTimeoutError
...
def _sync(form, env=None):
    ...
    for sub_env in env:
        ...
        for fn_ in cache:
            ...
            if os.path.isfile(dest):
                ...
                srch = hashlib.md5(
                    bonneville.utils.fopen(fn_, 'r').read()
                ).hexdigest()
                dsth = hashlib.md5(
                    bonneville.utils.fopen(dest, 'r').read()
                ).hexdigest()
                if srch != dsth:
    ...
    if touched:
        ...
        with bonneville.utils.fopen(mod_file, 'a+') as ofile:
            ofile.write('')
    return ret
...
def running():
    ...
    for fn_ in os.listdir(proc_dir):
        ...
        if not isinstance(data, dict):
            ...
            continue
        if not bonneville.utils.process.os_is_running(data['pid']):
...
def regen_keys():
    ...
    auth = bonneville.crypt.SAuth(__opts__)
...
def cmd(tgt,
    ...
    '''
    if ssh:
        client = bonneville.client.SSHClient(
