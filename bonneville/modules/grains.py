...
import bonneville.utils.dictupdate
...
def get(key, default=''):
    ...
    '''
    return bonneville.utils.traverse_dict(__grains__, key, default)
...
def items(sanitize=False):
    ...
    '''
    if bonneville.utils.is_true(sanitize):
        out = dict(__grains__)
...
def item(*args, **kwargs):
    ...
    for arg in args:
        try:
            ...
            pass
    if bonneville.utils.is_true(kwargs.get('sanitize')):
...
def setval(key, val, destructive=False):
    ...
    if os.path.isfile(gfn):
        with bonneville.utils.fopen(gfn, 'rb') as fp_:
            try:
    ...
    with bonneville.utils.fopen(gfn, 'w+') as fp_:
        fp_.write(cstr)
    fn_ = os.path.join(__opts__['cachedir'], 'module_refresh')
    with bonneville.utils.fopen(fn_, 'w+') as fp_:
        fp_.write('')
    # Sync the grains
...
def filter_by(lookup_dict, grain='os_family', merge=None):
    ...
    if merge:
        bonneville.utils.dictupdate.update(ret, merge)
