...
def compound(tgt):
    ...
    matcher = bonneville.minion.Matcher(__opts__, __salt__)
    try:
...
def ipcidr(tgt):
    ...
    '''
    matcher = bonneville.minion.Matcher({'grains': __grains__}, __salt__)
    try:
...
def pillar(tgt, delim=':'):
    ...
    .. code-block:: bash
        ...
        salt '*' match.pillar 'clone_url|https://github.com/saltstack/bonneville.git' delim='|'

    .. versionchanged:: 0.16.4
        ``delim`` argument added
    '''
    matcher = bonneville.minion.Matcher({'pillar': __pillar__}, __salt__)
    try:
