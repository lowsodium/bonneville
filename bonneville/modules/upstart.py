...
.. warning::
    ...
    :mod:`rh_service <bonneville.modules.rh_service>` module should be used, as it
...
def __virtual__():
    ...
    if __grains__['os'] in ('Ubuntu', 'Linaro', 'elementary OS'):
        ...
        if os.path.isfile(debian_initctl):
            initctl_version = bonneville.modules.cmdmod._run_quiet(debian_initctl + ' version')
...
def _default_runlevel():
    ...
    try:
        with bonneville.utils.fopen('/etc/init/rc-sysinit.conf') as fp_:
            for line in fp_:
    ...
    try:
        with bonneville.utils.fopen('/etc/inittab') as fp_:
            for line in fp_:
    ...
    try:
        ...
        with bonneville.utils.fopen('/proc/cmdline') as fp_:
            for line in fp_:
...
def _get_service_exec():
    ...
    bonneville.utils.check_or_die(executable)
    return executable
