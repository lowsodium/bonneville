...
def ping(host):
    ...
    '''
    cmd = 'ping -c 4 {0}'.format(bonneville.utils.network.sanitize_host(host))
...
def traceroute(host):
    ...
    '''
    ret = []
    if not bonneville.utils.which('traceroute'):
        log.info('This minion does not have traceroute installed')
        return ret

    cmd = 'traceroute {0}'.format(bonneville.utils.network.sanitize_host(host))
...
def ip_addrs(interface=None, include_loopback=False, cidr=None):
    ...
    '''
    addrs = bonneville.utils.network.ip_addrs(interface=interface,
                                        include_loopback=include_loopback)
    if cidr:
        return [i for i in addrs if bonneville.utils.network.in_subnet(cidr, [i])]
    else:
        return addrs
