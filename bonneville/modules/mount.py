...
from bonneville.utils import which as _which
from bonneville.exceptions import CommandNotFoundError, CommandExecutionError
...
def _active_mountinfo(ret):
    ...
    with bonneville.utils.fopen(filename) as ifile:
        for line in ifile:
            comps = line.split()
...
def fstab(config='/etc/fstab'):
    ...
    if not os.path.isfile(config):
        return ret
    with bonneville.utils.fopen(config) as ifile:
        for line in ifile:
            if line.startswith('#'):
                # Commented
                continue
...
def rm_fstab(name, config='/etc/fstab'):
    ...
    lines = []
    try:
        with bonneville.utils.fopen(config, 'r') as ifile:
            for line in ifile:
                if line.startswith('#'):
                    # Commented
                    lines.append(line)
    ...
    try:
        with bonneville.utils.fopen(config, 'w+') as ofile:
...
def set_fstab(
    ...
    if change:
        if not bonneville.utils.test_mode(test=test, **kwargs):
            try:
                with bonneville.utils.fopen(config, 'w+') as ofile:
...
def swaps():
    ...
    '''
    ret = {}
    with bonneville.utils.fopen('/proc/swaps') as fp_:
        for line in fp_:
