...
def _parse_pkg_meta(path):
    ...
    def parse_rpm(path):
        try:
            from bonneville.modules.yumpkg5 import __QUERYFORMAT, _parse_pkginfo
...
def _repack_pkgs(pkgs):
    ...
    return dict(
        [
            ...
            for x, y in bonneville.utils.repack_dictlist(pkgs).items()
        ]
    )
...
def version(*names, **kwargs):
    ...
    ret = {}
    versions_as_list = \
        bonneville.utils.is_true(kwargs.get('versions_as_list'))
    pkg_glob = False
    if len(names) != 0:
