...
import bonneville.utils.timed_subprocess
import bonneville.grains.extra
from bonneville._compat import string_types
from bonneville.exceptions import CommandExecutionError, TimedProcTimeoutError
...
DEFAULT_SHELL = bonneville.grains.extra.shell()['shell']
...
def _chugid(runas):
    ...
    #
    # 08:46:32,161 [bonneville.loaded.int.module.cmdmod:276 ][DEBUG   ] stderr: Traceback (most recent call last):
    ...
    # 08:46:17,481 [bonneville.loaded.int.module.cmdmod:59  ][DEBUG   ] Switching user 0 -> 1008 and group 0 -> 1012 if needed
    #
...
def _render_cmd(cmd, cwd, template):
    ...
    if template not in bonneville.utils.templates.TEMPLATE_REGISTRY:
        raise CommandExecutionError(
    ...
    def _render(contents):
        ...
        tmp_path_fn = bonneville.utils.mkstemp()
        with bonneville.utils.fopen(tmp_path_fn, 'w+') as fp_:
            fp_.write(contents)
        ...
        bonneville.utils.safe_rm(tmp_path_fn)
        if not data['result']:
...
def _run(cmd,
    ...
    if runas and bonneville.utils.is_windows():
    ...
    if not bonneville.utils.is_true(quiet):
    ...
    try:
        proc = bonneville.utils.timed_subprocess.TimedProc(cmd, **kwargs)
...
def script(source,
    ...
    path = bonneville.utils.mkstemp(dir=cwd, suffix=os.path.splitext(source)[1])
...
def which(cmd):
    ...
    .. code-block:: bash
        ...
        salt '*' cmd.which cat
    '''
    return bonneville.utils.which(cmd)
...
def which_bin(cmds):
    ...
    '''
    return bonneville.utils.which_bin(cmds)
...
def exec_code(lang, code, cwd=None):
    ...
    '''
    codefile = bonneville.utils.mkstemp()
    with bonneville.utils.fopen(codefile, 'w+') as fp_:
        fp_.write(code)
