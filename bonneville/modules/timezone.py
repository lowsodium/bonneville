...
def get_zone():
    ...
    if 'Arch' in __grains__['os_family']:
        ...
        with bonneville.utils.fopen('/etc/timezone', 'r') as ofh:
            return ofh.read()
...
def set_zone(timezone):
    ...
    if 'Arch' in __grains__['os_family']:
        ...
        with bonneville.utils.fopen('/etc/timezone', 'w') as ofh:
            ofh.write(timezone)
    elif 'Gentoo' in __grains__['os_family']:
        with bonneville.utils.fopen('/etc/timezone', 'w') as ofh:
            ofh.write(timezone)
...
def zone_compare(timezone):
    ...
    with bonneville.utils.fopen(zonepath, 'r') as fp_:
        usrzone = hashlib.md5(fp_.read()).hexdigest()

    with bonneville.utils.fopen(tzfile, 'r') as fp_:
...
def get_hwclock():
    ...
    if 'Arch' in __grains__['os_family']:
        ...
        if os.path.isfile('/etc/rtc_config'):
            with bonneville.utils.fopen('/etc/rtc_config', 'r') as fp_:
                for line in fp_:
