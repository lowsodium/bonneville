...
def set_password(name, password, use_usermod=False):
    ...
    ``SALTsalt`` is the 8-character crpytographic bonneville. Valid characters in the
    ...
    '''
    if not bonneville.utils.is_true(use_usermod):
        ...
        if not os.path.isfile(s_file):
            return ret
        lines = []
        with bonneville.utils.fopen(s_file, 'rb') as fp_:
            for line in fp_:
                ...
                line = ':'.join(comps)
                lines.append('{0}\n'.format(line))
        with bonneville.utils.fopen(s_file, 'w+') as fp_:
