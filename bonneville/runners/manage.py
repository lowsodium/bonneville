...
def status(output=True):
    ...
    key = bonneville.key.Key(__opts__)
    keys = key.list_keys()
    ...
    if output:
        bonneville.output.display_output(ret, '', __opts__)
    return ret
...
def down():
    ...
    for minion in ret:
        bonneville.output.display_output(minion, '', __opts__)
    return ret
...
def versions():
    ...
    comps = bonneville.__version__.split('-')
    if len(comps) == 3:
        master_version = '-'.join(comps[0:2])
    else:
        master_version = bonneville.__version__
