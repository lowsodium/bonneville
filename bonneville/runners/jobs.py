...
def active():
    ...
    '''
    ret = {}
    client = bonneville.client.LocalClient(__opts__['conf_file'])
    ...
    bonneville.output.display_output(ret, 'yaml', __opts__)
    return ret
...
def lookup_jid(jid, ext_source=None):
    ...
    if __opts__['ext_job_cache'] or ext_source:
        ...
        mminion = bonneville.minion.MasterMinion(__opts__)
        ...
        for minion in data:
            ...
            if 'out' in data[minion]:
                out = data[minion]['out']
        bonneville.output.display_output(ret, out, __opts__)
        return ret
    ...
    for top in os.listdir(job_dir):
        ...
        for final in os.listdir(t_path):
            ...
            if not os.path.isfile(loadpath):
                continue
            load = serial.load(bonneville.utils.fopen(loadpath, 'rb'))
            jid = load['jid']
            ret[jid] = {'Start Time': bonneville.utils.jid_to_time(jid),
                        'Function': load['fun'],
                ...
                for host in os.listdir(hosts_path):
                    ...
                    if os.path.isdir(host_path):
                        ...
                        return_data = serial.load(
                            bonneville.utils.fopen(returnfile, 'rb')
                        )
