# -*- coding: utf-8 -*-
'''
Execute overstate functions
'''

# Import pytohn libs
from __future__ import print_function

# Import bonneville libs
import bonneville.overstate
import bonneville.output


def over(env='base', os_fn=None):
    '''
    Execute an overstate sequence to orchestrate the executing of states
    over a group of systems

    CLI Examples:

    .. code-block:: bash

        salt-run state.over
        salt-run state.over env=dev /root/overstate.sls
    '''
    stage_num = 0
    overstate = bonneville.overstate.OverState(__opts__, env, os_fn)
    for stage in overstate.stages_iter():
        if isinstance(stage, dict):
            # This is highstate data
            print('Stage execution results:')
            for key, val in stage.items():
                if '_|-' in key:
                    bonneville.output.display_output(
                            {'error': {key: val}},
                            'highstate',
                            opts=__opts__)
                else:
                    bonneville.output.display_output(
                            {key: val},
                            'highstate',
                            opts=__opts__)
        elif isinstance(stage, list):
            # This is a stage
            if stage_num == 0:
                print('Executing the following Over State:')
            else:
                print('Executed Stage:')
            bonneville.output.display_output(stage, 'overstatestage', opts=__opts__)
            stage_num += 1
    return overstate.over_run


def sls(mods, env='base', test=None, exclude=None):
    '''
    Execute a state run from the master, used as a powerful orchestration
    system.

    CLI Examples:

    .. code-block:: bash

        salt-run state.sls webserver
        salt-run state.sls webserver env=dev test=True
    '''
    __opts__['file_client'] = 'local'
    minion = bonneville.minion.MasterMinion(__opts__)
    running = minion.functions['state.sls'](mods, env, test, exclude)
    ret = {minion.opts['id']: running}
    bonneville.output.display_output(ret, 'highstate', opts=__opts__)
    return ret


def show_stages(env='base', os_fn=None):
    '''
    Display the stage data to be executed

    CLI Examples:

    .. code-block:: bash

        salt-run state.show_stages
        salt-run state.show_stages env=dev /root/overstate.sls
    '''
    overstate = bonneville.overstate.OverState(__opts__, env, os_fn)
    bonneville.output.display_output(
            overstate.over,
            'overstatestage',
            opts=__opts__)
    return overstate.over
