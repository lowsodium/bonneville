...
def query(hyper=None, quiet=False):
    ...
    for info in client.cmd_iter('virtual:physical',
        ...
        ret.update(chunk)
        if not quiet:
            bonneville.output.display_output(chunk, 'virt_query', __opts__)
...
def list(hyper=None, quiet=False):
    ...
    for info in client.cmd_iter('virtual:physical',
        ...
        ret.update(chunk)
        if not quiet:
            bonneville.output.display_output(chunk, 'virt_list', __opts__)
...
def hyper_info(hyper=None):
    ...
    bonneville.output.display_output(data, 'nested', __opts__)
    return data
...
def init(name, cpu, mem, image, hyper=None, seed=True, nic='default', install=True):
    ...
    if seed:
        ...
        kv = bonneville.utils.virt.VirtKey(hyper, name, __opts__)
        kv.authorize()
